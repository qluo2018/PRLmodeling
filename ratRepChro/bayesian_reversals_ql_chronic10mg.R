...
# Adopted by Qiang Luo from Rudolf Cardinal for JK, 19 Dec 2017
# . ~/activate_venv.sh
...
# =============================================================================
# - Groups: SSRI, placebo
# - Sessions: 1-7 (within subject) but we pool the data together to have more data to increase the reliability of the estimations
...
FIT_CACHE_DIR <- paste(THIS_SCRIPT_DIR, "fitcache10mg", sep="/")
...
SOURCEDATA_FILENAME <-  "cleaned_subchronic_10mg_bari.csv" #"cleaned_chronic_5mg_bari.csv"
fulldata <- data.table(read.csv(SOURCEDATA_FILENAME))

# we ran three versions of this program
# the first version pooling 5 sessions togetheer
# the second version pooling the last 2 sessions together
# the third version pooling the first 3 sessions together
# indexNow <- which(fulldata$session > 3)
num_session <- 5
# fulldata <- fulldata[-indexNow,] # delete all data from the first 3 sessions
...
setnames(
    ...
    new=c(
        "sess",        # 1-5  
        "subject",
        "group",       # 0--Vehicle, 1--SSRI   # between subject
        "reinforced",
        "choice_was_optimal",
...
standata <- list(
    ...
    N_SESS = num_session, # 5 sessions within subject
    N_TRIALS = nrow(fulldata),  # TOTAL number of trials
    N_STIMULI = 2,
    N_GROUPS = 2, # sham and ssri 2 groups between subject
...
test_extra_contrasts_winner <- function(cutoff)
{
    ...
    plac <- stanfunc$sampled_values_from_stanfit(stanfit, "side_stickiness_by_group[1]")
    ssri <- stanfunc$sampled_values_from_stanfit(stanfit, "side_stickiness_by_group[2]")
    ct_ssriminusplac <- ssri - plac
    hdi_ct_ssriminusplac <- stanfunc$hdi(ct_ssriminusplac, hdi_proportion = cutoff)
    print('stickiness')
    print(hdi_ct_ssriminusplac)


    plac <- stanfunc$sampled_values_from_stanfit(stanfit, "reward_rate_by_group[1]")
    ssri <- stanfunc$sampled_values_from_stanfit(stanfit, "reward_rate_by_group[2]")
    ct_ssriminusplac <- ssri - plac
    hdi_ct_ssriminusplac <- stanfunc$hdi(ct_ssriminusplac, hdi_proportion = cutoff)
    print('reward rate')
    print(hdi_ct_ssriminusplac)

    plac <- stanfunc$sampled_values_from_stanfit(stanfit, "punish_rate_by_group[1]")
    ssri <- stanfunc$sampled_values_from_stanfit(stanfit, "punish_rate_by_group[2]")
    ct_ssriminusplac <- ssri - plac
    hdi_ct_ssriminusplac <- stanfunc$hdi(ct_ssriminusplac, hdi_proportion = cutoff)
    print('punish rate')
    print(hdi_ct_ssriminusplac)

    plac <- stanfunc$sampled_values_from_stanfit(stanfit, "reinf_sensitivity_by_group[1]")
    ssri <- stanfunc$sampled_values_from_stanfit(stanfit, "reinf_sensitivity_by_group[2]")
    ct_ssriminusplac <- ssri - plac
    hdi_ct_ssriminusplac <- stanfunc$hdi(ct_ssriminusplac, hdi_proportion = cutoff)
    print('reinf_sensitivity')
    print(hdi_ct_ssriminusplac)
...
save_group_level_params <- function(stanfit)
{
    ...
    write.csv(d, file="output/grp_para_bariChron10mg.csv")
}
...
save_individual_subject_params <- function()
{
    ...
    write.csv(d, file="output/indi_para_bariChron10mg.csv")
}
...
presentation2 <- function(model = NULL)
{
    ...
    #hidbound <- c(0.90, 0.90, 0.90, 0.75)
    p2_ct <- stanfunc$plot_multiple_stanfit_parameters_vstack(  # in the stanfunc lib, there is a sign reversal 
        model,
        list(
            list(name="ct_reward_rate_sham_minus_depletion", desc="Reward rate: SSRI - Vehicle"),
            list(name="ct_punish_rate_sham_minus_depletion", desc="Punishment rate: SSRI - Vehicle"),
            list(name="ct_reinf_sensitivity_sham_minus_depletion", desc="Reinf. sensitivity: SSRI - Vehicle"),
            list(name="ct_side_stickiness_sham_minus_depletion", desc="Side (location) stickiness: SSRI - Vehicle")
        ),
        #hidbound,
        title="Rat chronic SSRI experiment",
        reverse=TRUE
    )
    ggsave("output/ct_bariChron10mg.pdf", p2_ct, width=17.5, height=7.5, units="cm")
}
...
presentation3 <- function(model = NULL)
{
    ...
    p2_ct <- stanfunc$plot_multiple_stanfit_parameters_vstack(
        model,
        list(
            list(name="ct_reinforcement_rate_sham_minus_depletion", desc="Reward rate: SSRI - Vehicle"),
            list(name="ct_reinf_sensitivity_sham_minus_depletion", desc="Reinf. sensitivity: SSRI - Vehicle"),
            list(name="ct_side_stickiness_sham_minus_depletion", desc="Side (location) stickiness: SSRI - Vehicle")
        ),
        title="Rat chronic SSRI experiment",
    ...
    ggsave("/output/ct_bariChron10mgModel3.pdf", p2_ct, width=17.5, height=7.5, units="cm")
}
...
#human_comparison()
#print(comparison)
# write.csv(comparison, file="output/comp_bariChron10mg.csv")

...
Pleft <- vector(mode="double", length=nTrials)
Pright <- vector(mode="double", length=nTrials)
Qleft <- vector(mode="double", length=nTrials)
Qright <- vector(mode="double", length=nTrials)
choice <- vector(mode="double", length=nTrials)
outcome <- vector(mode="double", length=nTrials)
active <- vector(mode="double", length=nTrials)
wCrit <- vector(mode="double", length=(nTrials+1))
...
#Random vectors
set.seed(rat)
r.outcome <- runif(nTrials,0,1)
r.choice <- runif(nTrials,0,1)
...
#active <- c(rep(2, nTrials)) #for testing purposes
...
#    run_models("3")
#    s <- summarize_for_group_level_params(fit3)
#    cols <- c("parameter", "mean", "sd")  # use the mean

#    s[grep("^reinforcement_rate_by_group.*", parameter), ..cols]
#    s[grep("^reinf_sensitivity_by_group.*", parameter), ..cols]
#    s[grep("^side_stickiness_by_group.*", parameter), ..cols]
...
beta = 5.45     # sham 5.48  depletion  5.45     # reinforcement sensitivity
kappa = 0.01    # sham 0.31 depletion 0.01       # side stickiness
alpha = 0.50    # sham 0.50   depletion  0.50      # reinformcement learning rate
alpha_g = alpha
alpha_l = alpha
...
wins[rat] <- sum(outcome[1:nTrials-1])
winStay[rat] <- sum(outcome[1:(nTrials-1)]==1 & choice[1:(nTrials-1)]==choice[2:nTrials])
loseShift[rat] <- sum(outcome[1:(nTrials-1)]==0 & choice[1:(nTrials-1)]!=choice[2:nTrials])
losses[rat] <- sum(outcome[1:(nTrials-1)]==0 & choice[1:(nTrials-1)]>0)
...
write.csv(result, file = "test_ql_depl.csv", row.names = F)
...
plot.new()
plot(1:nTrials, choice-1)
#points(1:nTrials, Pright, col="red")
points(1:nTrials, Pleft, col="magenta")
title(main = c(paste0('alpha_g=', alpha_g, "; alpha_l=",alpha_l, "; beta=",beta, 
                      ' ; stickiness=', kappa,
                      "; #reversal=",sum(wCrit==8))))

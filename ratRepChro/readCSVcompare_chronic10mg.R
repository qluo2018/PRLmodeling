...
data.beh <- read.csv("cleaned_subchronic_10mg_bari.csv")

# repetition in the records for rat 4 in session 6
# indexNow <- which(data.beh$session == 6 & data.beh$patient == 4)
# print(length(indexNow))
# data.beh <- data.beh[-indexNow[seq(1,400,2)],]
# ##################
...
# levels(s2$group) <- c("Vehicle", "SSRI")
# p1 <- ggplot(s2,
...
levels(behaviour$group) <- c("Vehicle", "SSRI")
...
#p123 <- ggarrange(p1, p2, p3, p11, p21, p31, nrow = 2, ncol = 3) + theme(text=element_text(size=16,  family="Arial"))
#ggsave('expbeh10mg.pdf', p123)

############################
# individual parameters
############################
source('~/qluo/compbehmodeling/LSD/support/toolkits.R')
source('bayesian_reversals_ql_chronic.R')
run_models("2s")
para.indiviudal <- extract_per_subject_params()
groupname <- as.factor(para.indiviudal$groupname)
groupname <- relevel(groupname, "Vehicle")
...
write.csv(r, "indiviudalBehParaCorr_R_chronic10mg.csv")
write.csv(p, "indiviudalBehParaCorr_p_chronic10mg.csv")

q <- array(p.adjust(p, method = "BH"), dim = dim(p), dimnames = dimnames(p))

write.csv(q, "indiviudalBehParaCorr_fdr10mg.csv")
...
# simulations
#############################
source('bayesian_reversals_ql_chronic10mg.R')
run_models('2')
...
colnames(x.sd) <- c('alpha_g', 'alpha_l', 'beta', 'kappa')
x.file<- 'test_ql_chro10mgV3.csv'
simulatewithstats.cri(x.mean, x.sd, x.file)




data0 <- read.csv("test_ql_veic10mgV3.csv", header = T)
data1 <- read.csv("test_ql_chro10mgV3.csv", header = T)
...
levels(data$group) <- c("Vehicle", "SSRI")
...
#compare_means(loseShiftRate ~ group,  data = data)
#wilcox.test(data$loseShiftRate[which(data$group=='Vehicle')], data$loseShiftRate[which(data$group=='SSRI')])
# Perorm pairwise comparisons
simcomp <- array(rep(0,19*2*1), dim = c(19,2,1), dimnames = list(c(colnames(data)[c(1:19)]),
             c('wt', 'p.value'), c('Vehicle vs SSRI')))

my_comparisons <- list(c('Vehicle', 'SSRI'))
...
write.csv(simcomp, file = 'simbehresult10mg.csv')
...
#ggarrange(p31, p32, p33, nrow = 1) + theme(text=element_text(size=16,  family="Arial"))

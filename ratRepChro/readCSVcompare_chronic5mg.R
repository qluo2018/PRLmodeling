...
source('bayesian_reversals_ql_chronic5mg.R')


# test the association between the individual parameters and the behavioural indeces

data.beh <- read.csv("cleaned_chronic_5mg_bari.csv")
...
# ggsave('expbeh5mg.pdf', p123, width=17, height=7)
...
write.csv(r, "indiviudalBehParaCorr_R_chronic5mg.csv")
write.csv(p, "indiviudalBehParaCorr_p_chronic5mg.csv")

q <- array(p.adjust(p, method = "BH"), dim = dim(p), dimnames = dimnames(p))

write.csv(q, "indiviudalBehParaCorr_fdr5mg.csv")
...
colnames(x.sd) <- c('alpha_g', 'alpha_l', 'beta', 'kappa')
x.file<- 'test_ql_ssri5mgV3.csv'
simulatewithstats.cri(x.mean, x.sd, x.file)




data0 <- read.csv("test_ql_vehi5mgV3.csv", header = T)
data1 <- read.csv("test_ql_ssri5mgV3.csv", header = T)
...
write.csv(simcomp, file = 'simbehresult5mg.csv')
...
# p321 <- ggarrange(p31, p32, p33, nrow = 1) + theme(text=element_text(size=16,  family="Arial"))
# ggsave('simbehresult5mg.pdf', p321, width=12, height=7)

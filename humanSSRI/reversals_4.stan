...
parameters {
    ...
    real<lower=0, upper=1> phi_by_group[N_GROUPS];
    real<lower=0, upper=1> rho_by_group[N_GROUPS];
    real<lower=0> beta_by_group[N_GROUPS];
...
model {
    ...
    sampleBeta_ARR_lp(phi_by_group, PRIOR_BETA_SHAPE1, PRIOR_BETA_SHAPE2);
    sampleBeta_ARR_lp(rho_by_group, PRIOR_BETA_SHAPE1, PRIOR_BETA_SHAPE2);
    sampleGamma_ARR_lp(beta_by_group, PRIOR_GAMMA_ALPHA_FOR_REINF_SENSITIVITY_MEAN, PRIOR_GAMMA_BETA_FOR_REINF_SENSITIVITY_MEAN);  // positive only
    ...
    // ========================================================================
    {
        ...
        for (t in 1:N_TRIALS) {
            ...
            if (d != sess[t]) {
                ...
                phi = phi_by_group[g] + phi_subject_effect[s];
                rho = rho_by_group[g] + rho_subject_effect[s];
                beta = beta_by_group[g] + beta_subject_effect[s];
            ...
            p_choose_stim1[t] = softmaxNthInvTemp(v_stim_value, beta, STIM1);
...
generated quantities {
    ...
    real ct_phi_sham_minus_depletion =
    phi_by_group[SHAM_GROUP] -
    phi_by_group[DEPL_GROUP];
    
    
    real ct_rho_sham_minus_depletion =
    rho_by_group[SHAM_GROUP] -
    rho_by_group[DEPL_GROUP];
    
    real ct_beta_sham_minus_depletion =
    beta_by_group[SHAM_GROUP] -
    beta_by_group[DEPL_GROUP];

...
parameters {
    ...
    real<lower=0, upper=1> reinforcement_rate_by_group[N_GROUPS];
...
model {
    ...
    sampleBeta_ARR_lp(reinforcement_rate_by_group, PRIOR_BETA_SHAPE1, PRIOR_BETA_SHAPE2);
    ...
    // ========================================================================
    {
        ...
        for (t in 1:N_TRIALS) {
            ...
            if (d != sess[t]) {
              ...
              reinforcement_rate_final = reinforcement_rate_by_group[g] + reinforcement_rate_subject_effect[s];
...
generated quantities {
    ...
    real ct_reinforcement_rate_sham_minus_depletion  =
    reinforcement_rate_by_group[SHAM_GROUP] -
    reinforcement_rate_by_group[DEPL_GROUP];
    ...
    real reinforcement_rate_final[N_SUBJECTS];
    real reinf_sensitivity_final[N_SUBJECTS];
    real side_stickiness_final[N_SUBJECTS];
    int g;
    for (s in 1:N_SUBJECTS) {
      g = group_membership[s];
      reinforcement_rate_final[s] = reinforcement_rate_by_group[g] + reinforcement_rate_subject_effect[s];
      reinf_sensitivity_final[s] = reinf_sensitivity_by_group[g] + reinf_sensitivity_subject_effect[s];
      side_stickiness_final[s] = side_stickiness_by_group[g] + side_stickiness_subject_effect[s];
      
      reinforcement_rate_final[s] = bound(reinforcement_rate_final[s], 0, 1);
      reinf_sensitivity_final[s] = boundLower(reinf_sensitivity_final[s], 0);
      // side_stickiness_final may be negative
    }

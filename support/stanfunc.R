                                          ...
                                          ...)
{
  ...
  if (!is.null(par_regex)) {
    for (par_included in par_regex){
      pars <- pars[grepl(par_included, pars)]
    }
  }
  if (!is.null(par_exclude_regex)) {
    for (par_excluded in par_exclude_regex){
      pars <- pars[!grepl(par_excluded, pars)]
    }
  }
  if (length(pars) == 0) {
...
)
{
  ...
  s <- stanfunc$summary_by_par_regex(fit, pars = pars, probs = probs,
                                     par_regex = par_regex, par_exclude_regex = par_exclude_regex)
...
stanfunc$plot_multiple_stanfit_parameters_vstack <- function(
  ...
  inner_hdi_proportion = 0.75, # as suggested by a reviewer
    ...
    if (!is.na(compare_to)) {
      ...
      if (colour_hdi) {
        if (d$inner_hdi_excludes_zero[i]) {
          d$innerhdi_colour[i] <- "blue"
          d$main_colour[i] <- "blue"
        }
...
stanfunc$plot_multiple_stanfit_parameters_vstack3 <- function(
  fit,
  params,  # list(list(name = name1, desc = desc1), list(name = name2, desc = desc2)...)
  # ... inner bit being a list because c() can't hold expressions properly
  hidbound,
  ...
  colour_hdi = TRUE)
{
  ...
  for (i in 1:n) {
    ...
    inner_hdi_limits <- hdi(sampled_values, hidbound[i])
    d$inner_hdi_lower[i] <- inner_hdi_limits[1]
    d$inner_hdi_upper[i] <- inner_hdi_limits[2]
    ...
    if (!is.na(compare_to)) {
      ...
      if (colour_hdi) {
        if (d$inner_hdi_excludes_zero[i]) {
          if ( hidbound[i] == 0.75 ){
            d$innerhdi_colour[i] <- "brown"
            d$main_colour[i] <- "brown"
          } else if ( hidbound[i] == 0.80 ){
            d$innerhdi_colour[i] <- "purple"
            d$main_colour[i] <- "purple"
          }else if ( hidbound[i] == 0.85 ){
            d$innerhdi_colour[i] <- "blue"
            d$main_colour[i] <- "blue"
          }else if ( hidbound[i] == 0.90 ){
...
stanfunc$plot_multiple_stanfit_parameters_vstack_9585 <- function(
  fit,
  params,  # list(list(name = name1, desc = desc1), list(name = name2, desc = desc2)...)
  # ... inner bit being a list because c() can't hold expressions properly
  inner_hdi_proportion = 0.85,
  outer_hdi_proportion = 0.95,
  xlab = bquote(
    paste(
      "mean"
  ...
  colour_hdi = TRUE)
{
  ...
  for (i in 1:n) {
    ...
    para_dose_100 <- sampled_values_from_stanfit(fit, paste0(parname,"[3]"))
    para_dose_0 <- sampled_values_from_stanfit(fit, paste0(parname,"[1]"))
    sampled_values <- para_dose_100 - para_dose_0
